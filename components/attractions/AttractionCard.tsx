  ...
  return (
    <article className="group relative h-full min-w-0 transition duration-300 hover:-translate-y-1">
      <Link href={href} className="block h-full">
        <GlassPanel frost={false} className="flex h-full flex-col transition group-hover:ring-1 group-hover:ring-primary/40">
          <div className="relative aspect-4/3 overflow-hidden rounded-t-[inherit]">
            <Image src={attraction.imageSrc} alt={localizedName(attraction.name, loc)} fill sizes="(min-width:1280px) 29vw, (min-width:768px) 45vw, 92vw" className="object-cover transition duration-500 group-hover:scale-105" />
            <div aria-hidden className="from-ink/60 absolute inset-x-0 top-0 h-24 bg-linear-to-b to-transparent" />
          </div>
      ...
      {detailBasePath.startsWith("/user") ? <SavePlaceButton className="absolute end-3 top-3 z-10" place={{ id: attraction.slug, category: "attractions", name: attraction.name, governorate: attraction.governorate, imageSrc: attraction.imageSrc, href }} /> : null}
    </article>
  );
}
