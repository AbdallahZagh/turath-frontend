...
export function EventBookingCheckout({ eventId, initialSessionId, initialQuantity }: { eventId: string; initialSessionId?: string; initialQuantity: number }): ReactNode {
  ...
  const tiers: SelectOption[] = (session?.tiers ?? []).map((item) => ({ value: item.id, label: tt(item.id), hint: `${formatMoney(item.priceSyp)} · ${t("remaining", { count: item.remaining })}`, disabled: item.remaining < quantity }));
