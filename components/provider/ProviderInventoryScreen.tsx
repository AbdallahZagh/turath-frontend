...
function RowActions({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }): ReactNode {
  ...
  return (
    <div className="flex justify-end gap-1">
      <button
        type="button"
        className="text-prose-muted hover:bg-option-hover hover:text-prose grid size-8 place-items-center rounded-lg"
        aria-label={t("edit")}
        onClick={onEdit}
      >
      ...
      <button
        type="button"
        className="text-prose-muted hover:bg-destructive/10 hover:text-destructive grid size-8 place-items-center rounded-lg"
        aria-label={t("delete")}
        onClick={onDelete}
      >
    ...
    <div className="flex flex-col gap-5">
      ...
      {canEdit ? null : (
        <GlassPanel className="flex items-start gap-3 p-5">
          <span className="bg-warning/12 text-warning grid size-10 shrink-0 place-items-center rounded-xl">
            <Eye className="size-5" aria-hidden />
          </span>
          <div>
            <p className="text-prose text-sm font-semibold">{t("readOnlyTitle")}</p>
            <p className="text-prose-muted mt-1 text-sm leading-6">{t("readOnlyDescription")}</p>
          </div>
        </GlassPanel>
      )}
      {query.isPending ? (
      ...
      {query.data ? (
        <InventoryBody inventory={query.data} locale={locale} actions={actions} />
      ) : null}
      {canEdit ? (
        <>
            ...
            onConfirm={() => {
              ...
              remove.mutate(
                { category: deleteTarget.category, id: deleteTarget.id },
                {
                  ...
                  onError: () => toast.error(t("saveFailedTitle"), t("saveFailedBody")),
                },
              );
            }}
            title={t("deleteTitle")}
...
function InventoryBody({ inventory, locale, actions }: InventoryBodyProps): ReactNode {
  if (inventory.category === "hotels")
    return <HotelInventory rooms={inventory.rooms} locale={locale} actions={actions} />;
  if (inventory.category === "restaurants")
    return (
      <RestaurantInventory
  ...
  if (inventory.category === "trips")
    return <TripInventory trips={inventory.trips} locale={locale} actions={actions} />;
  if (inventory.category === "events")
    return <EventInventory sessions={inventory.sessions} locale={locale} actions={actions} />;
...
function SectionHeader({
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <h2 className="font-heading text-prose text-2xl font-semibold">{title}</h2>
        <p className="text-prose-muted mt-1 text-sm">{description}</p>
      </div>
      {action}
    </div>
  );
...
function actionsColumn<T>(
  actions: InventoryActions | undefined,
  header: string,
  renderActions: (item: T, actions: InventoryActions) => ReactNode,
): TableColumn<T>[] {
  if (!actions) return [];
  ...
  const columns: TableColumn<HotelRoom>[] = [
    {
      ...
      cell: (item) => <span className="font-semibold">{inventoryItemName(item, locale)}</span>,
    },
    {
      id: "occupancy",
      header: t("columns.occupancy"),
      align: "end",
      cell: (item) => formatCount(item.occupancy, locale),
    },
    {
      id: "quantity",
      header: t("columns.quantity"),
      align: "end",
      cell: (item) => formatCount(item.quantity, locale),
    },
    {
      id: "price",
      header: t("columns.price"),
      align: "end",
      cell: (item) => <span className="whitespace-nowrap">{formatMoney(item.priceSyp)}</span>,
    },
    {
      id: "amenities",
      header: t("columns.amenities"),
      cell: (item) => (
        <div className="flex flex-wrap gap-1">
          {item.amenities.map((value) => (
            <Badge key={value}>{t(`form.amenity.${value}`)}</Badge>
          ))}
        </div>
      ),
    },
    ...actionsColumn<HotelRoom>(actions, t("columns.actions"), (item, { onEdit, onDelete }) => (
      <RowActions
        onEdit={() => onEdit({ kind: "hotels", item })}
        onDelete={() =>
          onDelete({ category: "hotels", id: item.id, name: inventoryItemName(item, locale) })
        }
      />
    )),
  ];
    ...
    <section className="space-y-4">
      ...
      {rooms.length ? (
        <Table
          ...
          caption={t("sections.hotels.title")}
        />
      ) : (
        <InventoryEmpty
          action={actions ? () => actions.onEdit({ kind: "hotels", item: null }) : undefined}
        />
      )}
    </section>
  );
  ...
  const columns: TableColumn<RestaurantTable>[] = [
    {
      ...
      cell: (item) => <span className="font-semibold">{item.label}</span>,
    },
    {
      id: "capacity",
      header: t("columns.capacity"),
      align: "end",
      cell: (item) => formatCount(item.capacity, locale),
    },
    { id: "zone", header: t("columns.zone"), cell: (item) => t(`form.zoneOptions.${item.zone}`) },
    ...actionsColumn<RestaurantTable>(
      actions,
      t("columns.actions"),
      (item, { onEdit, onDelete }) => (
        <RowActions
          onEdit={() => onEdit({ kind: "restaurants", item })}
          onDelete={() => onDelete({ category: "restaurants", id: item.id, name: item.label })}
        />
      ),
    ),
  ];
    ...
    <section className="space-y-4">
      ...
      <GlassPanel className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        {actions ? (
          <Button
            variant="outline"
            size="sm"
            onClick={() => actions.onEdit({ kind: "restaurantSchedule", slots })}
          >
            {t("editSlots")}
          </Button>
        ) : null}
      ...
      {tables.length ? (
        <Table
          ...
          rows={tables}
          getRowId={(item) => item.id}
          caption={t("sections.restaurants.title")}
        />
      ) : (
        <InventoryEmpty
          action={actions ? () => actions.onEdit({ kind: "restaurants", item: null }) : undefined}
        />
      )}
    </section>
  );
  ...
  const columns: TableColumn<TripOffering>[] = [
    {
      ...
      cell: (item) => (
        <div>
          <p className="font-semibold">{inventoryItemName(item, locale)}</p>
          <p className="text-prose-muted mt-1 max-w-sm truncate text-xs">
            {locale === "ar" ? item.pickupAr : item.pickupEn}
          </p>
        </div>
      ),
    },
    { id: "date", header: t("columns.date"), cell: (item) => formatMediumDate(item.date, locale) },
    {
      ...
      align: "end",
      cell: (item) => t("seatsValue", { left: item.seatsLeft, total: item.capacity }),
    },
    {
      id: "price",
      header: t("columns.price"),
    ...
    ...actionsColumn<TripOffering>(actions, t("columns.actions"), (item, { onEdit, onDelete }) => (
      <RowActions
        onEdit={() => onEdit({ kind: "trips", item })}
        onDelete={() =>
          onDelete({ category: "trips", id: item.id, name: inventoryItemName(item, locale) })
        }
      />
    )),
  ];
    ...
    <section className="space-y-4">
      ...
      {trips.length ? (
        <Table
          ...
          rows={trips}
          getRowId={(item) => item.id}
          caption={t("sections.trips.title")}
        />
      ) : (
        <InventoryEmpty
          action={actions ? () => actions.onEdit({ kind: "trips", item: null }) : undefined}
        />
      )}
    </section>
  );
  ...
  const columns: TableColumn<EventSession>[] = [
    {
      ...
      cell: (item) => (
        <div>
          <p className="font-semibold">{inventoryItemName(item, locale)}</p>
          <Badge className="mt-1">{t(`form.${item.tier}`)}</Badge>
        </div>
      ),
    },
    {
      ...
      cell: (item) => formatBookingWhen({ start: item.date, time: item.time }, locale),
    },
    {
      id: "availability",
      header: t("columns.available"),
      align: "end",
      cell: (item) => t("seatsValue", { left: item.available, total: item.capacity }),
    },
    {
      id: "price",
      header: t("columns.price"),
    ...
    ...actionsColumn<EventSession>(actions, t("columns.actions"), (item, { onEdit, onDelete }) => (
      <RowActions
        onEdit={() => onEdit({ kind: "events", item })}
        onDelete={() =>
          onDelete({ category: "events", id: item.id, name: inventoryItemName(item, locale) })
        }
      />
    )),
  ];
    ...
    <section className="space-y-4">
      ...
      {sessions.length ? (
        <Table
          ...
          rows={sessions}
          getRowId={(item) => item.id}
          caption={t("sections.events.title")}
        />
      ) : (
        <InventoryEmpty
          action={actions ? () => actions.onEdit({ kind: "events", item: null }) : undefined}
        />
      )}
    </section>
  );
...
function GuideInventory({
  guide,
  locale,
  actions,
}: {
  guide: Extract<ProviderInventory, { category: "guides" }>["guide"];
  locale: Locale;
  actions?: InventoryActions;
}): ReactNode {
  const t = useTranslations("provider.inventory");
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <GlassPanel className="p-6">
          <dl className="grid gap-5 sm:grid-cols-2">
            <div>
              <dt className="text-prose-muted text-xs">{t("form.licenseNumber")}</dt>
              <dd className="text-prose mt-1 font-semibold">{guide.licenseNumber}</dd>
            </div>
            <div>
              <dt className="text-prose-muted text-xs">{t("form.languages")}</dt>
              <dd className="mt-1 flex flex-wrap gap-1">
                {guide.languages.map((value) => (
                  <Badge key={value}>{t(`form.language.${value}`)}</Badge>
                ))}
              </dd>
            </div>
            <div>
              <dt className="text-prose-muted text-xs">{t("form.hourlySyp")}</dt>
              <dd className="text-prose mt-1 font-semibold">{formatMoney(guide.hourlySyp)}</dd>
            </div>
            <div>
              <dt className="text-prose-muted text-xs">{t("form.fullDaySyp")}</dt>
              <dd className="text-prose mt-1 font-semibold">{formatMoney(guide.fullDaySyp)}</dd>
            </div>
          </dl>
        </GlassPanel>
        <GlassPanel className="p-6">
          <p className="text-prose-muted text-xs">{t("specialties")}</p>
          <p className="text-prose mt-2 text-sm leading-7">
