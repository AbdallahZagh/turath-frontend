...
export function AttractionCatalog({
  initialFilters,
  detailBasePath = "/attractions",
}: {
  initialFilters: AttractionFilters;
  detailBasePath?: string;
}): ReactNode {
  const t = useTranslations("attractions");
  const tf = useTranslations("attractions.filters");
  const tGov = useTranslations("landing.governorates");
  const [filters, setFilters] = useState(initialFilters);
  const query = useAttractions(filters);
          ...
          <Select
            ...
            onChange={(value) =>
              setFilters({
                ...filters,
                governorate:
                  value === "all" ? undefined : (value as AttractionFilters["governorate"]),
              })
            }
          />
