  ...
  return (
    <article className="group h-full min-w-0">
      <Link href={result.detailHref} className="block h-full">
        ...
        >
          <div className={`relative overflow-hidden ${compact ? "min-h-32" : "aspect-4/3"}`}>
            <Image
              src={result.imageSrc}
              alt={localizedName(result.name, loc)}
              fill
              sizes={compact ? "7rem" : "(min-width:1280px) 25vw, (min-width:768px) 45vw, 92vw"}
          ...
          <div className="flex min-w-0 flex-1 flex-col p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <Badge>{t(`categories.${result.category}`)}</Badge>
                <h2
            ...
            <div className="border-border mt-auto flex items-end justify-between gap-3 border-t pt-3">
              ...
              <div className="text-end">
                <p className="text-prose font-semibold">
                  {result.priceSyp === 0 ? t("free") : formatMoney(result.priceSyp)}
                </p>
                <p className="text-prose-muted text-[0.7rem]">
