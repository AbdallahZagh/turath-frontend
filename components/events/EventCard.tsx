...
export function EventCard({ event, detailBasePath = "/events" }: { event: TourismEvent; detailBasePath?: string }): ReactNode {
  ...
  return <article className="group h-full min-w-0 transition duration-300 ease-out hover:-translate-y-1"><Link href={`${detailBasePath}/${event.id}`} className="block h-full min-w-0"><GlassPanel frost={false} className="h-full transition duration-300 group-hover:ring-1 group-hover:ring-primary/40">
