...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> { const { id } = await params; const item = await getRestaurant(id); return item ? { title: `${item.name.en} | Turath`, description: item.shortDescription.en } : { title: "Restaurant | Turath" }; }
